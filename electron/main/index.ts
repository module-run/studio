...
import ApiManagerMain from "./../lib/api-manager-main";
...
Storage.init({
    root: path.join(Env.userData, 'storage')
})
File.init({
    root: path.join(Env.userData, 'data')
})
Log.init({
    root: Env.userData
})
...
Log.info('Starting')
Log.info('UserDataDir', Env.userData)
...
async function createWindow() {
    if (hasSplashWindow) {
        ...
        if (VITE_DEV_SERVER_URL) {
            splashWin.loadURL(path.join(VITE_DEV_SERVER_URL, 'splash.html'))
        } else {
    ...
    IPC.init(win)
    const apiManager = new ApiManagerMain()
    apiManager.register(app, win, ipcMain)
}
